...
if ($action === 'get_sent_requests') {
    ...
    $st = $db->prepare("
        SELECT pc.pedcon_id AS request_id, u.usuar_id, u.usuar_nome, u.usuar_foto_perfil
        FROM pedido_conexao pc
        JOIN usuario u ON u.usuar_id = pc.pedcon_usuar_destinatario_id
        WHERE pc.pedcon_usuar_remetente_id=? AND pc.pedcon_estado='pendente'
    ");
    $st->execute([$userId]);
...
if ($action === 'get_new_connections') {
    ...
    $st = $db->prepare("
        ...
        WHERE pc.pedcon_estado='aceite'
          AND (pc.pedcon_usuar_remetente_id=? OR pc.pedcon_usuar_destinatario_id=?)
          AND NOT EXISTS (
              SELECT 1 FROM post WHERE post_connect_id = pc.pedcon_id LIMIT 1
          )
    ");
    $st->execute([$userId, $userId, $userId]);
    ok($st->fetchAll(PDO::FETCH_ASSOC));
